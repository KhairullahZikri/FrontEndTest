...
const arr1 = [
  ...
  function result(arr1, arr2) {
    ...
    for (let i = 0; i < arr2.length; i++) {
      if(arr2[i].score > arr1[0].min_score && arr2[i].score < arr1[1].min_score){
        const student = {};
        student.name = arr2[i].name;
        student.id = arr2[i].id;
        student.score = arr2[i].score;
        student.status_test = arr1[0].status;
        output.push(student);
      }
      else if(arr2[i].score > arr1[1].min_score && arr2[i].score < arr1[2].min_score){
        const student = {};
        student.name = arr2[i].name;
        student.id = arr2[i].id;
        student.score = arr2[i].score;
        student.status_test = arr1[1].status;
        output.push(student);
      }
      else if(arr2[i].score > arr1[2].min_score){
        const student = {};
        student.name = arr2[i].name;
        student.id = arr2[i].id;
        student.score = arr2[i].score;
        student.status_test = arr1[2].status;
        output.push(student);
      }
    }
